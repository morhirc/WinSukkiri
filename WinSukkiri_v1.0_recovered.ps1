...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
 Title="Winすっきり v1.0" Height="680" Width="1120" MinHeight="560" MinWidth="900" ResizeMode="CanResize" WindowStartupLocation="CenterScreen" Background="#F4F6FA" FontFamily="Segoe UI, Yu Gothic UI">
 <Window.Resources>
...
function Show-Home{
    ...
    $SummaryText.Text='Winすっきり v1.0';$DetailRisk.Text='安全';$DetailAdmin.Text='不要';$DetailRestart.Text='すぐ反映';$DetailTitle.Text='ホーム';$DetailBody.Text='Winすっきりはキーボードのファンクションキーに頼らず、補助機能も画面からマウスで開けます。現在のWindowsが目的の状態かを優先して判定します。'
}
...
function Save-LastSnapshot{$o=[ordered]@{app='WinSukkiri';version='1.0';created=(Get-Date).ToString('s');settings=[ordered]@{}};foreach($s in $settings){if($toggleMap[$s.Id].IsEnabled){$o.settings[$s.Id]=[bool]$initialMap[$s.Id]}};$o|ConvertTo-Json -Depth 6|Set-Content $lastSnapshotPath -Encoding UTF8}
...
function Prompt-RestartExplorerForShortcutSetting {
    if(-not $script:ShortcutSuffixChanged){return}
    $script:ShortcutSuffixChanged=$false
    $r=[System.Windows.MessageBox]::Show(
        "ショートカット名の設定を反映するため、Explorerを再起動しますか？`n`n［はい］: Explorerを再起動します。`n［いいえ］: 今は再起動しません。",
    ...
    if($r -eq 'Yes'){
        ...
        $StatusText.Text='ショートカット名の設定を変更しました。Explorer再起動は後で行えます。'
    }
}
