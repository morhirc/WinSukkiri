...
$regHiberboot='HKLM:\SYSTEM\CurrentControlSet\Control\Session Manager\Power'
$regHiberbootNative='HKLM\SYSTEM\CurrentControlSet\Control\Session Manager\Power'
...
function Get-FastStartupDisabled {
    try{return ([int](Get-ItemPropertyValue -Path $regHiberboot -Name 'HiberbootEnabled' -ErrorAction Stop) -eq 0)}catch{return $false}
}
function Set-FastStartupEnabled([bool]$Enabled){
    $value=if($Enabled){1}else{0}
    Set-HklmDwordElevated $regHiberbootNative 'HiberbootEnabled' $value
}
...
$settings=@(
 ...
 [pscustomobject]@{Id='DisableFastStartup';Cat='パフォーマンス';Title='高速スタートアップを無効にする';Subtitle='シャットダウン時の高速スタートアップをOFFにし、完全終了を優先します。';Risk='注意';NeedsAdmin=$true;Restart='不要';Recommended=$false;Exists={$true};IsApplied={Get-FastStartupDisabled};Apply={Set-FastStartupEnabled $false};Revert={Set-FastStartupEnabled $true};Detail='Windowsの高速スタートアップ（HiberbootEnabled）をOFFにします。休止状態とは別項目なので、休止状態を残したまま高速スタートアップだけ無効化できます。休止状態自体が無効な場合、高速スタートアップは設定値に関係なく利用されません。OFFに戻すと高速スタートアップを有効設定へ戻します。'},
...
# タスクバーアイコン対策。
# WinSukkiri.exe は専用PowerShellプロセスでWPF画面を動かすため、WPFの Icon プロパティだけでは
# 環境によって powershell.exe のアイコンが残ることがあります。
# そのため AppUserModelID と Win32 の WM_SETICON の両方を設定します。
try {
    if (-not ('WinSukkiri.NativeIcon' -as [type])) {
        Add-Type @'
using System;
using System.Runtime.InteropServices;
namespace WinSukkiri {
...
'@
    ...
    [void][WinSukkiri.NativeIcon]::SetCurrentProcessExplicitAppUserModelID('morhirc.WinSukkiri')
} catch {}

$script:NativeIconHandle = [IntPtr]::Zero
$script:SetNativeWindowIcon = {
    try {
        $windowIconPath = Join-Path $PSScriptRoot 'WinSukkiri.ico'
        if (-not (Test-Path -LiteralPath $windowIconPath)) { return }
        ...
        $LR_DEFAULTSIZE = 0x40
        $hIcon = [WinSukkiri.NativeIcon]::LoadImage([IntPtr]::Zero, $windowIconPath, $IMAGE_ICON, 0, 0, ($LR_LOADFROMFILE -bor $LR_DEFAULTSIZE))
        if ($hIcon -ne [IntPtr]::Zero) {
            $script:NativeIconHandle = $hIcon
            $WM_SETICON = 0x0080
            [void][WinSukkiri.NativeIcon]::SendMessage($hwnd, $WM_SETICON, [IntPtr]1, $hIcon) # ICON_BIG
            [void][WinSukkiri.NativeIcon]::SendMessage($hwnd, $WM_SETICON, [IntPtr]0, $hIcon) # ICON_SMALL
        }
    } catch {}
}

# HWND作成後に確実に設定する。
$window.Add_SourceInitialized({ & $script:SetNativeWindowIcon })
...
function Save-LastSnapshot{$o=[ordered]@{app='WinSukkiri';version='1.1';created=(Get-Date).ToString('s');settings=[ordered]@{}};foreach($s in $settings){if($toggleMap[$s.Id].IsEnabled){$o.settings[$s.Id]=[bool]$initialMap[$s.Id]}};$o|ConvertTo-Json -Depth 6|Set-Content $lastSnapshotPath -Encoding UTF8}
